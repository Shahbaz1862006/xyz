...
export function SecurityShield3D({ dragRotation, isDragging, reducedMotion = false }: Props) {
  ...
  */
  ...
  const frontMat = useMemo(() => new THREE.MeshStandardMaterial({
    color: new THREE.Color('#0B55CC'), metalness: 0.72, roughness: 0.28,
    emissive: new THREE.Color('#061b4c'), emissiveIntensity: 0.16,
  }), [])
  ...
  const panelMat = useMemo(() => new THREE.MeshStandardMaterial({
    color: new THREE.Color('#1470F0'), metalness: 0.60, roughness: 0.35,
    emissive: new THREE.Color('#0b3f9b'), emissiveIntensity: 0.13,
  }), [])
  ...
  useFrame(({ clock }) => {
    ...
    if (isDragging) {
      ...
      // Keep the lit face toward the camera. A continuous full rotation exposes
      // the unlit back of this layered shield and makes it appear to disappear.
      groupRef.current.rotation.y +=
        (Math.sin(t * 0.45) * 0.38 - groupRef.current.rotation.y) * 0.018
