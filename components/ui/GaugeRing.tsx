    ...
    <div ref={ref} className={cn('flex flex-col items-center gap-3', className)}>
      <div className="relative" style={{ width: size, height: size }}>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="font-grifter font-bold text-xl" style={{ color: 'var(--on-surface)' }}>{value}%</span>
        </div>
      </div>
      <span className="text-sm font-poppins text-center" style={{ color: 'var(--on-surface-2)' }}>{label}</span>
    </div>
  )
}
